...
isotherms_shaded = ax.contourf(t.metpy.x, t.metpy.y, tf_smoothed, transform=t.metpy.cartopy_crs, cmap='jet', levels=np.arange(30, 111, 1), extend='both')
...
#plt.savefig('plots/rtma/temp/latest.png')
plt.show()
...
fig, ax = plt.subplots(figsize=(12, 8), subplot_kw={'projection': ccrs.PlateCarree()})
isodrosotherms_shaded = ax.contourf(tdf.metpy.x, tdf.metpy.y, tdf_smoothed, transform=tdf.metpy.cartopy_crs, cmap='YlGn', levels=np.arange(45, 86, 5), extend='both')
fig.colorbar(isodrosotherms_shaded, label='Degrees Fahrenheit', fraction=0.046, pad=0.04)
isodrosotherms_outlined = ax.contour(tdf.metpy.x, tdf.metpy.y, tdf_smoothed, transform=tdf.metpy.cartopy_crs, colors='Black', levels=np.arange(45, 86, 5), extend='both')
...
#plt.savefig('plots/rtma/dewpoint/latest.png')
plt.show()
